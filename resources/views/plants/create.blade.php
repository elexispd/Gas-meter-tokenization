...
<form id="quickForm" method="post" action="{{ route('plant.store') }}">
    @csrf
...
<textarea name="address" id="address" class="form-control" cols="30" rows="10" placeholder="Enter location address...">{{ old('address') }}</textarea>
    @error('address')
        <span class="text-danger"> {{ $message }}</span>
    @enderror
</div>
...
<select class="form-control" name="tenant">
    ...
    @foreach ($tenants as $tenant)
        <option value="{{ $tenant->id }}" {{ old('tenant') == '1' ? 'selected' : '' }}> {{ $tenant->first_name }} {{ $tenant->last_name }}</option>
    @endforeach
